<template>
  <b-container>
    <b-row align-h="center">
      <h3 class="mb-3">bookmarked trades</h3>
      <b-col cols="12">
        ...
        <div v-else>
          <div v-if="trades.length === 0">
            <b-row align-h="center" class="mt-3"
              ><p>You don't have any bookmarked trades!</p></b-row
            >
            <b-row align-h="center"
              ><b-button variant="collectr-primary" to="/">
                View recent trades
              </b-button></b-row
            >
          </div>
...
export default {
  ...
  async fetch() {
    this.trades = await this.$axios.$get(
      `${this.$nuxt.context.$auth.user.id}/bookmarked/`
    )
  },
  fetchDelay: 500,
}
